# -*- coding: utf-8 -*-
"""
BERGE v7.3 - Couverture végétale RGB · herbes sèches + veto sol craquelé · GeoPackage
Profil par défaut stabilisé : veto sol craquelé renforcé (0.06 / 0.06 / 0.12).
...
class BergeV7Algorithm(QgsProcessingAlgorithm):
    ...
    # récupération de végétation sèche/fibreuse peu verte
    T_DRY_VEG_SCORE            = "T_DRY_VEG_SCORE"
    T_DRY_VEG_TEXTURE          = "T_DRY_VEG_TEXTURE"
    T_DRY_VEG_ENTROPY          = "T_DRY_VEG_ENTROPY"
    T_DRY_VEG_EDGE             = "T_DRY_VEG_EDGE"
    T_DRY_VEG_CHROMA           = "T_DRY_VEG_CHROMA"
    T_DRY_VEG_YELLOW           = "T_DRY_VEG_YELLOW"
    T_DRY_VEG_SAT              = "T_DRY_VEG_SAT"
    T_DRY_VEG_LUM_MIN          = "T_DRY_VEG_LUM_MIN"

    # récupération locale de végétation sèche clairsemée / paille
    DRY_FIBRE_WINDOW           = "DRY_FIBRE_WINDOW"
    T_DRY_FIBRE_DENSITY        = "T_DRY_FIBRE_DENSITY"
    T_DRY_FIBRE_YELLOW         = "T_DRY_FIBRE_YELLOW"
    T_DRY_FIBRE_CHROMA         = "T_DRY_FIBRE_CHROMA"
    T_DRY_FIBRE_SAT            = "T_DRY_FIBRE_SAT"
    T_STRAW_LIGHT_ANOMALY      = "T_STRAW_LIGHT_ANOMALY"

    # veto sol nu craquelé : réseau de fissures sombres sans fibres claires
    T_CRACK_DARK_DENSITY       = "T_CRACK_DARK_DENSITY"
    T_CRACK_GREEN_MAX          = "T_CRACK_GREEN_MAX"
    T_CRACK_LIGHT_MAX          = "T_CRACK_LIGHT_MAX"

    T_VEG_CONTEXT              = "T_VEG_CONTEXT"
    ...
    def initAlgorithm(self, config=None):
        ...
        # Récupération végétation sèche / fibreuse peu verte
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_VEG_SCORE,
            "Récupération herbes sèches : score composite minimal",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.42, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_VEG_TEXTURE,
            "Récupération herbes sèches : texture Lum minimale",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.26, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_VEG_ENTROPY,
            "Récupération herbes sèches : entropie locale minimale",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.30, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_VEG_EDGE,
            "Récupération herbes sèches : densité de bords minimale",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.25, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_VEG_CHROMA,
            "Récupération herbes sèches : texture chromatique minimale",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.08, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_VEG_YELLOW,
            "Récupération herbes sèches : indice jaune-brun minimal",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.25, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_VEG_SAT,
            "Récupération herbes sèches : saturation normalisée minimale",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.16, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_VEG_LUM_MIN,
            "Récupération herbes sèches : luminance minimale, pour éviter ombre/eau",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.10, minValue=0.0, maxValue=0.8
        ))

        # Récupération locale de végétation sèche clairsemée / paille
        self.addParameter(QgsProcessingParameterNumber(
            self.DRY_FIBRE_WINDOW,
            "Fenêtre de densité locale des fibres sèches/paille (pixels, impaire)",
            QgsProcessingParameterNumber.Integer,
            defaultValue=15, minValue=5, maxValue=61
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_FIBRE_DENSITY,
            "Récupération herbes sèches clairsemées : densité locale minimale de fibres",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.055, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_FIBRE_YELLOW,
            "Récupération herbes sèches clairsemées : seuil jaune-brun souple",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.25, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_FIBRE_CHROMA,
            "Récupération herbes sèches clairsemées : seuil chroma_texture souple",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.08, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_DRY_FIBRE_SAT,
            "Récupération herbes sèches clairsemées : saturation minimale des graines paille",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.18, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_STRAW_LIGHT_ANOMALY,
            "Récupération herbes sèches : contraste clair minimal des fibres par rapport au voisinage",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.018, minValue=0.0, maxValue=0.20
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_CRACK_DARK_DENSITY,
            "Veto sol craquelé : densité minimale de fissures sombres",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.06, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_CRACK_GREEN_MAX,
            "Veto sol craquelé : densité verte maximale autorisée",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.12, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_CRACK_LIGHT_MAX,
            "Veto sol craquelé : densité maximale de fibres claires",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.06, minValue=0.0, maxValue=1.0
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.T_VEG_CONTEXT,
            "Récupération contextuelle : densité locale minimale de voisinage végétalisé",
            QgsProcessingParameterNumber.Double,
            defaultValue=0.080, minValue=0.0, maxValue=1.0
        ))
        ...
        self.addParameter(QgsProcessingParameterNumber(
            ...
            QgsProcessingParameterNumber.Integer,
            defaultValue=8, minValue=0, maxValue=10000
        ))
        self.addParameter(QgsProcessingParameterBoolean(
    ...
    def _gradient_magnitude(self, arr, valid):
        """Magnitude de gradient centrée 3×3, NumPy pur. Sert à détecter les textures fibreuses."""
        ...
        try:
            for table in ('stats_pct', 'berge_parameters', 'berge_metadata_json'):
                ...
                cur.execute(
                    ...
                    (table, table, f'Table BERGE v7.3 : {table}')
                )
        except Exception:
            pass
    ...
    def _save_layer_style_to_gpkg(self, layer, gpkg_path, table_name, feedback):
        ...
        description = f'Style par défaut généré automatiquement par BERGE v7.3 pour {table_name}'
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        t_dryveg_score   = pf(self.T_DRY_VEG_SCORE)
        t_dryveg_texture = pf(self.T_DRY_VEG_TEXTURE)
        t_dryveg_entropy = pf(self.T_DRY_VEG_ENTROPY)
        t_dryveg_edge    = pf(self.T_DRY_VEG_EDGE)
        t_dryveg_chroma  = pf(self.T_DRY_VEG_CHROMA)
        t_dryveg_yellow  = pf(self.T_DRY_VEG_YELLOW)
        t_dryveg_sat     = pf(self.T_DRY_VEG_SAT)
        t_dryveg_lum_min = pf(self.T_DRY_VEG_LUM_MIN)
        dry_fibre_win    = self._odd(pi(self.DRY_FIBRE_WINDOW), 5)
        t_dryfibre_dens  = pf(self.T_DRY_FIBRE_DENSITY)
        t_dryfibre_yel   = pf(self.T_DRY_FIBRE_YELLOW)
        t_dryfibre_chr   = pf(self.T_DRY_FIBRE_CHROMA)
        t_dryfibre_sat   = pf(self.T_DRY_FIBRE_SAT)
        t_straw_light    = pf(self.T_STRAW_LIGHT_ANOMALY)
        t_crack_dark_dens= pf(self.T_CRACK_DARK_DENSITY)
        t_crack_green_max= pf(self.T_CRACK_GREEN_MAX)
        t_crack_light_max= pf(self.T_CRACK_LIGHT_MAX)
        t_veg_context    = pf(self.T_VEG_CONTEXT)

        min_pixels      = pi(self.MIN_PIXELS)
        black_as_nodata = pb(self.TREAT_BLACK_AS_NODATA)
        keep_intermed   = pb(self.KEEP_INTERMEDIATE)

        gpkg_out     = os.path.join(out_dir, f'{prefix}_BERGE_v7_3.gpkg')
        ...
        tmp_dir = tempfile.mkdtemp(prefix='berge_v7_1_')
        ...
        try:
            ...
            # ── 1. Découpage ────────────────────────────────────────────────
            feedback.pushInfo('BERGE v7.3 : découpage de l\'orthophoto par l\'emprise...')
            try:
            ...
            # ── 7c. Signature herbes sèches / végétation fibreuse peu verte ───
            # Le problème observé sur sol craquelé est que l'herbe sèche/brune peut
            # avoir un score spectral faible et une faible densité de micro-pixels verts.
            # On ajoute donc une signature indépendante du vert :
            #   - texture Lum forte,
            #   - entropie locale forte,
            #   - densité de bords/filaments,
            #   - présence chromatique jaune-brun ou texture chromatique.
            feedback.pushInfo('Calcul de la signature herbes sèches / fibreuses...')

            grad_lum = self._gradient_magnitude(lum, index_valid)
            edge_list = []
            for k in windows:
                edge_list.append(self._local_mean(grad_lum, index_valid, k))
            edge_raw = np.maximum.reduce(edge_list)
            del edge_list, grad_lum
            edge_density, edge_lo, edge_hi = norm('EDGE_DENSITY', edge_raw)

            yellow_brown = np.full((ysize, xsize), NODATA_F, dtype=np.float32)
            # Indice simple sur RGB normalisé : valeurs hautes = déficit de bleu,
            # typique pailles/herbes sèches ; moins discriminant seul, utile en combinaison.
            yellow_brown[index_valid] = (0.5 * (R[index_valid] + G[index_valid]) - B[index_valid])
            yellow_brown_n, yb_lo, yb_hi = norm('YELLOW_BROWN', yellow_brown)

            dry_veg_score = np.full((ysize, xsize), NODATA_F, dtype=np.float32)
            dry_veg_score[index_valid] = np.clip(
                0.25 * dry_texture[index_valid] +
                0.25 * ent_n[index_valid] +
                0.20 * edge_density[index_valid] +
                0.15 * yellow_brown_n[index_valid] +
                0.15 * chroma_tex_n[index_valid],
                0.0, 1.0
            )

            dry_veg_evidence = (
                (edge_density    >= t_dryveg_edge).astype(np.uint8) +
                (chroma_tex_n    >= t_dryveg_chroma).astype(np.uint8) +
                (yellow_brown_n  >= t_dryveg_yellow).astype(np.uint8) +
                (sat_n           >= t_dryveg_sat).astype(np.uint8)
            )
            dry_veg_candidate = (
                index_valid &
                (dry_veg_score >= t_dryveg_score) &
                (dry_texture   >= t_dryveg_texture) &
                (ent_n         >= t_dryveg_entropy) &
                (dry_veg_evidence >= 2) &
                (lum           >= t_dryveg_lum_min)
            )

            # v7.3 : signature locale des fibres sèches clairsemées + veto sol craquelé.
            # Le faux positif observé en v7.2 vient des fissures sombres du sol nu :
            # elles produisent beaucoup de bords et une forte densité locale, comme les herbes sèches.
            # On sépare donc deux familles de lignes :
            #   - fibres claires / paille : luminance localement positive ;
            #   - fissures sombres : luminance localement négative, peu de vert, faible chroma.
            lum_mean_fibre = self._local_mean(lum, index_valid, dry_fibre_win)
            lum_anomaly = np.full((ysize, xsize), NODATA_F, dtype=np.float32)
            lum_anomaly[index_valid] = lum[index_valid] - lum_mean_fibre[index_valid]

            light_fibre_seed = (
                index_valid &
                (lum_anomaly >= t_straw_light) &
                (edge_density >= max(0.18, 0.75 * t_dryveg_edge)) &
                (
                    ((yellow_brown_n >= t_dryfibre_yel) & (sat_n >= t_dryfibre_sat)) |
                    ((yellow_brown_n >= max(0.18, 0.80 * t_dryfibre_yel)) & (chroma_tex_n >= t_dryfibre_chr)) |
                    ((chroma_tex_n >= max(0.10, t_dryfibre_chr)) & (green_density >= 0.025))
                )
            )
            light_fibre_density = self._local_fraction(light_fibre_seed, index_valid, dry_fibre_win)

            dark_crack_seed = (
                index_valid &
                (lum_anomaly <= -t_straw_light) &
                (edge_density >= max(0.20, 0.75 * t_dryveg_edge)) &
                (green_density <= t_crack_green_max) &
                (spectral <= max(t_crack_spec, 0.36)) &
                (chroma_tex_n <= max(t_crack_chroma, 0.18))
            )
            dark_crack_density = self._local_fraction(dark_crack_seed, index_valid, dry_fibre_win)

            dry_fibre_seed = (
                index_valid &
                (lum >= t_dryveg_lum_min) &
                (dry_texture >= max(0.18, 0.70 * t_dryveg_texture)) &
                (edge_density >= max(0.18, 0.75 * t_dryveg_edge)) &
                (
                    light_fibre_seed |
                    (
                        ...
                        (
                            ((yellow_brown_n >= max(0.18, 0.80 * t_dryfibre_yel)) & (chroma_tex_n >= t_dryfibre_chr)) |
                            ((chroma_tex_n >= max(0.10, t_dryfibre_chr)) & (sat_n >= t_dryfibre_sat))
                        )
                    )
                )
            )
            dry_fibre_density = self._local_fraction(dry_fibre_seed, index_valid, dry_fibre_win)

            crack_dominates_light = dark_crack_density >= np.maximum(
                t_crack_dark_dens,
                1.35 * light_fibre_density
            )
            bare_cracked_soil_pre = (
                index_valid &
                (spectral <= max(t_crack_spec, 0.36)) &
                (green_density <= t_crack_green_max) &
                ...
                crack_dominates_light
            )

            cracked_soil_core = (
                index_valid &
                (spectral     <= t_crack_spec) &
                (green_density <= min(t_crack_green_max, 0.080)) &
                (chroma_tex_n <= min(t_crack_chroma, 0.16)) &
                (dry_texture  >= t_crack_tex) &
                (light_fibre_density <= t_crack_light_max) &
                (dark_crack_density >= 0.50 * t_crack_dark_dens)
            )
            dry_fibre_candidate = (
                index_valid &
                (dry_fibre_density >= t_dryfibre_dens) &
                (light_fibre_density >= max(0.50 * t_dryfibre_dens, 0.020)) &
                (dry_texture >= max(0.20, 0.70 * t_dryveg_texture)) &
                (ent_n >= max(0.22, 0.75 * t_dryveg_entropy)) &
                (lum >= t_dryveg_lum_min) &
                (~cracked_soil_core) &
                (~bare_cracked_soil_pre)
            )
            ...
            # ── 8. Score BERGE v7.3 ──────────────────────────────────────────
            feedback.pushInfo('Calcul du score BERGE v7.3...')
            ...
            # ── 9. Classification ────────────────────────────────────────────
            feedback.pushInfo('Classification du score BERGE v7.3...')
            ...
            # GF1 : récupération végétation sèche déjà un peu verte
            # (classe 1 → 2 si texture forte + quelques micro-pixels verts)
            dry_recovery = (
                index_valid &
                (classes == 1) &
            ...
            # On ne l'applique pas aux signatures fibreuses pour éviter d'effacer les pailles.
            soil_guard = (
                index_valid &
                (classes == 2) &
            ...
            # GF1b : récupération herbes sèches/fibreuses peu vertes.
            # La décision ne dépend pas de la couleur verte mais d'une texture végétale organisée.
            dry_vegetation_recovery = (
                index_valid &
                (classes == 1) &
                dry_veg_candidate
            )
            classes[dry_vegetation_recovery] = 2

            # GF1c v7.3 : récupération des fibres sèches clairsemées.
            # Le critère est local : quelques filaments paille/herbe dans une fenêtre suffisent
            # à basculer la zone en végétation sèche, sauf si l'on est dans le coeur du sol craquelé.
            dry_fibre_recovery = (
                index_valid &
                (classes == 1) &
                dry_fibre_candidate
            )
            classes[dry_fibre_recovery] = 2

            # GF1d v7.3 : soutien contextuel.
            # On comble les interstices entre filaments et micro-taches vertes, sans dilater
            # brutalement sur le sol nu : il faut un voisinage déjà végétalisé et une texture minimale.
            vegetation_context_seed = (
                index_valid &
                (
                    (classes >= 3) |
                    dry_recovery |
                    dry_vegetation_recovery |
                    dry_fibre_candidate |
                    ((classes == 2) & (green_density >= 0.050))
                )
            )
            vegetation_context = self._local_fraction(vegetation_context_seed, index_valid, dry_fibre_win)
            context_recovery = (
                index_valid &
                (classes == 1) &
                (vegetation_context >= t_veg_context) &
                (dry_texture >= max(0.18, 0.65 * t_dryveg_texture)) &
                (ent_n >= max(0.20, 0.65 * t_dryveg_entropy)) &
                (
                    (light_fibre_density >= 0.50 * t_dryfibre_dens) |
                    ((dry_veg_score >= max(0.0, t_dryveg_score - 0.08)) & (green_density >= 0.030)) |
                    (green_density >= 0.060)
                ) &
                (~cracked_soil_core) &
                (~bare_cracked_soil_pre)
            )
            classes[context_recovery] = 2

            # GF3 : sol craquelé (classe ≥2 → 1 si spectral faible + chroma faible + texture forte).
            # v7.3 : les vrais filaments clairs restent protégés, mais le réseau de fissures sombres
            # peut reprendre la main même si la v7.2 l'avait récupéré à tort en classe 2.
            bare_cracked_soil_veto = (
                bare_cracked_soil_pre &
                (green_density <= t_crack_green_max) &
                (light_fibre_density <= t_crack_light_max)
            )
            ...
            if keep_intermed:
                ...
                add_raster('chroma_texture', chroma_tex_n, gdal.GDT_Float32, -9999,
                           style_kind='continuous')
                add_raster('edge_density', edge_density, gdal.GDT_Float32, -9999,
                           style_kind='continuous')
                add_raster('yellow_brown', yellow_brown_n, gdal.GDT_Float32, -9999,
                           style_kind='continuous')
                add_raster('dry_vegetation_score', dry_veg_score, gdal.GDT_Float32, -9999,
                           style_kind='continuous')
                add_raster('dry_fibre_seed', dry_fibre_seed.astype(np.uint8), gdal.GDT_Byte, 0,
                           style_kind='binary_guard', style_label='1 · graines fibres sèches/paille')
                add_raster('dry_fibre_density', dry_fibre_density, gdal.GDT_Float32, -9999,
                           style_kind='continuous')
                add_raster('light_fibre_density', light_fibre_density, gdal.GDT_Float32, -9999,
                           style_kind='continuous')
                add_raster('dark_crack_density', dark_crack_density, gdal.GDT_Float32, -9999,
                           style_kind='continuous')
                add_raster('vegetation_context', vegetation_context, gdal.GDT_Float32, -9999,
                           style_kind='continuous')
                add_raster('guard_dry_vegetation', dry_vegetation_recovery.astype(np.uint8), gdal.GDT_Byte, 0,
                           style_kind='binary_guard', style_label='1 · récupération herbes sèches fibreuses')
                add_raster('guard_dry_fibre', dry_fibre_recovery.astype(np.uint8), gdal.GDT_Byte, 0,
                           style_kind='binary_guard', style_label='1 · récupération fibres sèches clairsemées')
                add_raster('guard_context_recovery', context_recovery.astype(np.uint8), gdal.GDT_Byte, 0,
                           style_kind='binary_guard', style_label='1 · récupération contextuelle végétation sèche')
                add_raster('guard_bare_cracked_soil', bare_cracked_soil_veto.astype(np.uint8), gdal.GDT_Byte, 0,
                           style_kind='binary_guard', style_label='1 · veto sol nu craquelé')
            ...
            for code, label in labels.items():
                rows.append((site, date, 'BERGE_v7_3', 'classe', code, label,
                             counts[code], round(pcts[code], 3)))
            rows.extend([
                (site, date, 'BERGE_v7_3', 'indicateur', None,
            ...
            effective_parameters = {
                ...
                'T_DRY_VEG_SCORE': t_dryveg_score,
                'T_DRY_VEG_TEXTURE': t_dryveg_texture,
                'T_DRY_VEG_ENTROPY': t_dryveg_entropy,
                'T_DRY_VEG_EDGE': t_dryveg_edge,
                'T_DRY_VEG_CHROMA': t_dryveg_chroma,
                'T_DRY_VEG_YELLOW': t_dryveg_yellow,
                'T_DRY_VEG_SAT': t_dryveg_sat,
                'T_DRY_VEG_LUM_MIN': t_dryveg_lum_min,
                'DRY_FIBRE_WINDOW': dry_fibre_win,
                'T_DRY_FIBRE_DENSITY': t_dryfibre_dens,
                'T_DRY_FIBRE_YELLOW': t_dryfibre_yel,
                'T_DRY_FIBRE_CHROMA': t_dryfibre_chr,
                'T_DRY_FIBRE_SAT': t_dryfibre_sat,
                'T_STRAW_LIGHT_ANOMALY': t_straw_light,
                'T_CRACK_DARK_DENSITY': t_crack_dark_dens,
                'T_CRACK_GREEN_MAX': t_crack_green_max,
                'T_CRACK_LIGHT_MAX': t_crack_light_max,
                'T_VEG_CONTEXT': t_veg_context,
                'MIN_PIXELS': min_pixels,
            ...
            metadata = {
                ...
                ) + ' + récupération herbes sèches par fibres claires locales + veto réseau de fissures sombres',
                ...
                'normalization_bounds': {
                    ...
                    'EDGE_DENSITY':  {'low': edge_lo,    'high': edge_hi},
                    'YELLOW_BROWN':  {'low': yb_lo,      'high': yb_hi},
                },
                'diagnostic_stats': {
                    ...
                    'edge_density':  pstats(edge_density),
                    'yellow_brown':  pstats(yellow_brown_n),
                    'dry_vegetation_score': pstats(dry_veg_score),
                    'dry_fibre_density': pstats(dry_fibre_density),
                    'light_fibre_density': pstats(light_fibre_density),
                    'dark_crack_density': pstats(dark_crack_density),
                    'vegetation_context': pstats(vegetation_context),
                },
                'guard_pixels': {
                    'dry_recovery_green': int(np.sum(dry_recovery)),
                    'dry_vegetation_recovery': int(np.sum(dry_vegetation_recovery)),
                    'dry_vegetation_candidate': int(np.sum(dry_veg_candidate)),
                    'dry_fibre_seed': int(np.sum(dry_fibre_seed)),
                    'light_fibre_seed': int(np.sum(light_fibre_seed)),
                    'dark_crack_seed': int(np.sum(dark_crack_seed)),
                    'dry_fibre_candidate': int(np.sum(dry_fibre_candidate)),
                    'dry_fibre_recovery': int(np.sum(dry_fibre_recovery)),
                    'context_recovery': int(np.sum(context_recovery)),
                    'bare_cracked_soil_veto': int(np.sum(bare_cracked_soil_veto)),
            ...
            feedback.pushInfo('BERGE v7.3 terminé')
            ...
            feedback.pushInfo(
                ...
                f'dry_green:{int(np.sum(dry_recovery)):,}  '
                f'dry_score:{int(np.sum(dry_vegetation_recovery)):,}  '
                f'dry_fibre:{int(np.sum(dry_fibre_recovery)):,}  '
                f'context:{int(np.sum(context_recovery)):,}  '
                f'bare_crack:{int(np.sum(bare_cracked_soil_veto)):,}  '
    ...
    def name(self):
        return 'berge_v7_3_vegetation_rgb_texture'

    def displayName(self):
        return 'BERGE v7.3 - RGB · herbes sèches · veto sol craquelé'
    ...
    def shortHelpString(self):
        return (
            'BERGE v7.3 mesure la couverture végétale de berges et fossés à partir d\'une orthophoto '
            'drone RGB.\n\n'
            'Améliorations vs BERGE v7 stable :\n'
            ...
            '• Récupération des herbes sèches/fibreuses peu vertes par texture, entropie, '
            'densité de bords, signature jaune-brun et densité locale de fibres claires.\n'
            '• Veto sol nu craquelé renforcé : fissures sombres dominantes, faible densité verte, '
            'peu de fibres claires.\n'
            '• Garde-fou eau libre / pixels très sombres → exclus des statistiques.\n'
